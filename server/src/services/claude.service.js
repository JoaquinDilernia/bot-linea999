...
function buildReclamoInstructions() {
  return `
IMPORTANTE — RECLAMOS: Cuando un pasajero reporta un reclamo (demora, mal trato, incidente en una unidad, cobro indebido) o un objeto perdido, conversá primero para juntar un título breve y una descripción clara (línea/servicio, fecha y horario aproximado si aplica). Antes o junto con el marcador, avisale EXPLÍCITAMENTE en tu texto que le estás registrando el reclamo en el sistema — nunca lo hagas en silencio. Recién ahí, en una línea separada (invisible para el cliente), agregá:
[CREAR_RECLAMO:{"titulo":"...","descripcion":"...","prioridad":"baja|media|alta|urgente"}]
El JSON tiene que ser válido y tener exactamente esas 3 claves. Si todavía no tenés información suficiente para un título/descripción claros, seguí preguntando antes de usar el marcador — nunca lo generes con datos vacíos o inventados. Después de crear el reclamo el sistema le va a dar al pasajero un número de reclamo automáticamente — vos no inventes ni digas ningún número, eso lo agrega el sistema después de tu respuesta.`;
}
