...
// Este bot opera en Argentina (UTC-3 fijo, sin horario de verano desde 2009).
...
router.get('/', async (req, res) => {
  try {
    ...
    const [snap, agentsSnap, areasSnap, urgentSnap, awaitingSnap] = await Promise.all([
      db.collection('bot-nuevaaurora_conversations')
        .where('updatedAt', '>=', startTs)
        .where('updatedAt', '<=', endTs)
        .get(),
      db.collection('bot-nuevaaurora_agents').get(),
      db.collection('bot-nuevaaurora_areas').get(),
      db.collection('bot-nuevaaurora_conversations').where('urgent', '==', true).get(),
      db.collection('bot-nuevaaurora_conversations').where('status', '==', 'escalated').get(),
    ]);
