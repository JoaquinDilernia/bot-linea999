...
router.get('/', async (req, res) => {
  try {
    const db = getDb();
    const doc = await db.collection('bot-nuevaaurora_config').doc(CONFIG_DOC).get();
...
router.put('/', requireAtLeastAtencionCliente, async (req, res) => {
  try {
    const db = getDb();
    await db.collection('bot-nuevaaurora_config').doc(CONFIG_DOC).set(
...
function getDefaultConfig() {
  return {
    botName: 'Asistente',
    botPersonality: `Respondés de forma amigable, natural y cercana — como lo haría una persona real del equipo de Transportes Nueva Aurora.\nUsás un tono cálido y profesional. Nunca robótico ni genérico.\nEscribís en español rioplatense (vos, che, etc.) con claridad.\nSos especialmente preciso con horarios, tarifas y recorridos — la gente te consulta antes de salir de viaje.\nSi no sabés algo, lo decís honestamente y ofrecés derivar a una persona.\nNunca inventás horarios, tarifas ni recorridos — solo usás los datos que te den. Si algo no está en la información que tenés, lo decís honestamente en vez de inventar o suponer.`,
    welcomeMessage: '¡Hola! Soy el asistente virtual de Transportes Nueva Aurora 🚌 Te puedo ayudar con horarios, tarifas, recorridos y más. ¿En qué te ayudo?',
